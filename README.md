...
> **v0.5.2 重点：默认 AI 配置切到 `gpt-5.5` / `gpt-image-2`，官方 Amazon 模板支持直接导入识别，变体正式提交流程的 offer-only 误判已修复。** 这次更新把“官方模板识别 -> 2.8 模板生成 -> 变体预览 -> 正式提交”的链路进一步打通，同时把常见 Amazon 报错补成更适合商家理解的中文提示。完整变更见 [CHANGELOG.md](CHANGELOG.md)。
...
- 支持 OpenAI Responses / Chat Completions 与 Gemini generateContent
